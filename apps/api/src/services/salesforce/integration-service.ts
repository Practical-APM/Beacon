...
function parseMetadata(raw: Record<string, unknown> | null | undefined): SalesforceIntegrationMetadata {
  const fallback = buildDefaultMetadata('sandbox', '', '');
  if (!raw || typeof raw !== 'object') return fallback;
  return {
    ...fallback,
    ...(raw as Partial<SalesforceIntegrationMetadata>),
    fieldMappings: {
      ...fallback.fieldMappings,
      ...((raw.fieldMappings as SalesforceIntegrationMetadata['fieldMappings']) ?? {}),
    },
...
function toMetadataJson(metadata: SalesforceIntegrationMetadata): Record<string, unknown> {
  return metadata as unknown as Record<string, unknown>;
}

export async function getSalesforceIntegration(db: Database, tenantId: string) {
  return withTenantContext(db, tenantId, async () => {
    const [row] = await db
      .select()
      .from(integrations)
      .where(and(eq(integrations.tenantId, tenantId), eq(integrations.source, 'salesforce')))
      .limit(1);
    return row ?? null;
  });
}
...
export async function upsertSalesforceIntegration(
  ...
  return withTenantContext(db, tenantId, async () => {
    ...
    const existing = await getSalesforceIntegration(db, tenantId);
...
export async function getSalesforceCredentials(
  ...
  return JSON.parse(decrypted) as SalesforceCredentials;
}
...
export async function saveSalesforceCredentials(
  ...
  const integration = await getSalesforceIntegration(db, tenantId);
  if (!integration) throw notFound('Salesforce integration not found');
...
export async function updateSalesforceMetadata(
  db: Database,
  tenantId: string,
  metadata: SalesforceIntegrationMetadata,
...
export async function disconnectSalesforce(db: Database, tenantId: string) {
  const integration = await getSalesforceIntegration(db, tenantId);
  if (!integration) throw notFound('Salesforce integration not found');
  ...
  await withTenantContext(db, tenantId, async () => {
    await db
      ...
      .set({
        ...
        externalOrgId: null,
        lastError: null,
        metadata: toMetadataJson(buildDefaultMetadata('sandbox', '', '')),
        updatedAt: new Date(),
      })
      .where(eq(integrations.id, integration.id));
  });
...
export async function upsertCustomerFromSalesforce(
  db: Database,
  tenantId: string,
  accountId: string,
  accountName: string,
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(customers.externalSource, 'salesforce'),
...
export async function upsertProjectFromSalesforce(
  db: Database,
  tenantId: string,
  customerId: string,
  record: {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(projects.externalSource, 'salesforce'),
