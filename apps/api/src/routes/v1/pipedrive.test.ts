...
describe('Pipedrive integration (mock mode)', () => {
  const app = createApp();
  let tenantId = '';
  ...
  it('validates default field mappings as complete', () => {
    const { complete } = validatePipedriveFieldMappings({});
    expect(complete).toBe(false);
  });

  it.skipIf(!dbIntegrationReady())('connects mock Pipedrive and syncs deals', async () => {
    const connectRes = await app.request('/v1/integrations/pipedrive/mock-connect', {
      method: 'POST',
    ...
    const syncRes = await app.request('/v1/integrations/pipedrive/sync', {
      method: 'POST',
      headers: authHeaders('admin-a', tenantId),
    ...
    const statusRes = await app.request('/v1/integrations/pipedrive/status', {
      headers: authHeaders('admin-a', tenantId),
    });
  ...
  it.skipIf(!dbIntegrationReady())('detects Pipedrive portal change on reconnect', async () => {
    await app.request('/v1/integrations/pipedrive/mock-connect', {
      method: 'POST',
      headers: authHeaders('admin-a', tenantId),
      body: JSON.stringify({}),
    });

    const reconnectRes = await app.request('/v1/integrations/pipedrive/mock-connect', {
      method: 'POST',
      headers: authHeaders('admin-a', tenantId),
      body: JSON.stringify({ companyId: 'mock-portal-002' }),
    });
